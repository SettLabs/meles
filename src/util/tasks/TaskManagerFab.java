...
public class TaskManagerFab {
    ...
    public static void reloadTaskManager(TaskManager tm) {
        ...
        if (tm.getScriptPath().toString().endsWith(".xml")) {
            var dig = XMLdigger.goIn(tm.getScriptPath(), "meles", "tasklist");
            startDigging(dig, tm);
    ...
    private static boolean parseSet(XMLdigger taskset, AbstractBlock onFailure, TaskManager tm) {
        ...
        if (!isStep) {  // Everything starts at once so add splitter
            start.addNext(new SplitBlock(eventLoop,"sequential").setInterval(type.length == 2 ? type[1] : ""));
    ...
    private static AbstractBlock processTask(XMLdigger task, AbstractBlock start, TaskManager tm) {
        ...
        if( !reqAttr.isEmpty())
            req = ConditionBlock.build(reqAttr, rtvals, sharedMem).orElse(null);
    ...
    private static AbstractBlock handleTarget(XMLdigger task, TaskManager tm ) {
        ...
        return switch (target[0]) {
            ...
            case "email" -> {
                ...
                yield EmailBlock.sendEmail(Email.to(target[1]).subject(subs[0]).content(subs[1]).attachment(attachment));
            }
    ...
    private static AbstractBlock parseNode(XMLdigger node, TaskManager tm) {
        ...
        var eventLoop = tm.eventLoopGroup();
        var sharedMem = tm.sharedMem();
        return parseNode(node,rtvals,eventLoop,sharedMem);
    }
    public static AbstractBlock parseNodeList( XMLdigger dig, Rtvals rtvals, EventLoopGroup eventLoop ){
        AbstractBlock block=null;
        for( var sub : dig.digOut("*") ){
            var parsed = parseNode(sub,rtvals,eventLoop,null);
            if( block == null) {
                block=parsed;
            }else{
                block.setNext(parsed);
            }
        }
        return block;
    }
    private static AbstractBlock parseNode(XMLdigger node, Rtvals rtvals, EventLoopGroup eventLoop, ArrayList<NumericVal> sharedMem) {
        ...
        return switch (node.tagName("")) {
            ...
            case "email" -> {
                ...
                yield EmailBlock.sendEmail(Email.to(to).subject(subject).content(content).attachment(attach));
            }
    ...
    public static boolean addBlankTaskset(String id, Path scriptPath) {
        var fab = XMLfab.withRoot(scriptPath, "meles", "tasklist", "tasksets");
