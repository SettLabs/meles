...
public class TaskParser {
    ...
    private static OriginBlock doOriginBlock(Drawio.DrawioCell cell) {
        var ob = new OriginBlock(cell.melesId);
    ...
    private static BasicMathBlock doAlterValBlock(Drawio.DrawioCell cell, TaskTools tools, String id) {
        ...
        if (!op.equals("reset")) {
            if (NumberUtils.isParsable(source)) {
                if (source.contains(".")) {
                    sourceVal = RealVal.newVal("meles", "temp");
                    sourceVal.update(NumberUtils.toDouble(source));
                } else {
                    sourceVal = IntegerVal.newVal("meles", "temp");
    ...
    private static ControlBlock doControlBlock(Drawio.DrawioCell cell, TaskTools tools, String id) {
        ...
        if (stopCell != null || triggerCell != null) {
            var stopDas = stopCell == null ? "" : stopCell.melesId;
            var triggerDas = triggerCell == null ? "" : triggerCell.melesId;
    ...
    private static EmailBlock doEmailBlock(Drawio.DrawioCell cell, TaskTools tools, String id) {
        var blockId = alterId(id);
        Logger.info(blockId + " -> Processing EmailBlock");

        if (!cell.hasParam("to")) {
            Logger.error(blockId + " -> No 'to' specified for emailblock");
            return null;
        }
        if (!cell.hasParam("subject")) {
            Logger.error(blockId + " -> No subject specified for emailblock");
            return null;
        }

        Email em = Email.to(cell.getParam("to",""))
                    .subject( cell.getParam("subject","") )
                    .content( cell.getParam("content","") );
        var db = EmailBlock.sendEmail( em );

        db.id(alterId(id));
        addNext(cell, db, tools, "next", "ok", "done");
    ...
    private static FlagBlock doFlagBlock(Drawio.DrawioCell cell, TaskTools tools, String id) {
        ...
        if (!cell.hasParam("flag") && !cell.hasParam("flagval") && !cell.hasArrow("next")) {
    ...
    private static FlagBlock doOutputPin(Drawio.DrawioCell cell, TaskTools tools, String id) {
        var blockId = alterId(id);
    ...
    private static ReadingBlock doReaderBlock(Drawio.DrawioCell cell, TaskTools tools, String id) {
        ...
        reader.alterCheck(cell.getParam("check",""));
        reader.id(blockId);
        // Arrows
        addNext(cell, reader, tools, "received", "ok", "next");
        addAlt(cell, reader, tools, "timeout");
        addError(cell, reader, tools, "error","failure");
        return reader;
    }
    ...
    private static SplitBlock doSplitBlock(Drawio.DrawioCell cell, TaskTools tools, String id) {
        ...
        var sb = new SplitBlock(tools.eventLoop, cell.getParam("order","sequential"));
    ...
    private static TriggerGateBlock doTriggerGateBlock(Drawio.DrawioCell cell, TaskTools tools, String id) {
        ...
        addAlt(cell, tgb, tools, "retrigger", "timeout", "elapsed", "excess", "period passed");
        return tgb;
    }
    ...
    private static boolean addError(Drawio.DrawioCell cell, AbstractBlock block, TaskTools tools, String... labels) {
        tools.blocks.put(cell.drawId, block);
        for (var label : labels) {
            if (cell.hasArrow(label)) {
                var id = block.id();
                ...
                var existing = tools.blocks.get(targetId);
                if (existing == null) {
                    block.setErrorRouteBlock(createBlock(targetCell, tools, id));
                } else {
                    block.setErrorRouteBlock(existing);
                }
                return true;
            }
        }
