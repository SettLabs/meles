...
public class Drawio {
    ...
    public static HashMap<String, DrawioCell> parseTab(String tabName, XMLdigger diagram, HashMap<String, DrawioCell> cells) {
        ...
        for (var obj : diagram.digOut("object")) {
            if (!obj.attr("melestype", "").isEmpty())
                continue;
            if (obj.attr("meleslabel", "").isEmpty())
                continue;
            var label = obj.attr("label", "");
            if (label.isEmpty() || label.startsWith("%"))
                label = obj.attr("meleslabel", label);
        ...
        while (ars.hasNext()) {
            ...
            if (arrow.source != null && arrow.target != null) {
                if (arrow.target.type.endsWith("block")||arrow.target.type.endsWith("val")||arrow.target.type.endsWith("pin")) {
                    ...
                    if (arrow.source().drawId.equals(arrow.target().drawId)) {
                        Logger.warn("Looping arrow? id=" + arrow.source().drawId + "/" + arrow.source().melesId);
                        continue;
                    }
                    if( arrow.source.type.startsWith("split") ){ // Exception for splitblock and dot to allow multiple arrows without label
                        int a = 1;
                        while( a<50 ) {
                            if (!arrow.source.hasArrow("next_" + a)) {
                                arrow.source.addArrow("next_"+a, arrow.target);
                                blanks++;
                                ars.remove();
                                break;
                            }
                        ...
                        if (!arrow.source.hasArrow("next")) {
                            ...
                            Logger.warn("There's a shape with a blank and next arrow or two blanks! type=" + arrow.source().type + " id=" + arrow.source.drawId + "/" + arrow.source().melesId);
                        }
                    }
                }
            }
        ...
        Logger.info(tabName + " -> Found " + cellSize + " valid shapes, " + arrowCount
                + " unlabeled arrows of which " + arrows.size() + " are left after parsing.");
        return cells;
    }
    ...
    private static void processArrow(XMLdigger mxcell, String objLabel, HashMap<String, DrawioCell> cells, HashMap<String, Arrow> arrows) {
        ...
        var label = clean(mxcell.attr("value", objLabel));
    ...
    private static Optional<DrawioCell> parseShapeObjects(XMLdigger obj) {
        var drawId = obj.attr("id", "");
        var dasType = obj.attr("melestype", "");

        if (!dasType.isEmpty()) { // To know it's a parent node?
            var dasId = obj.attr("melesid", "");
    ...
    public static class DrawioCell {
        ...
        public DrawioCell(String drawId, String melesId, String type) {
            this.drawId = drawId;
