...
public class ValStoreFab {
    ...
    private static boolean reload(ValStore store, XMLdigger dig, String id, Rtvals rtvals) {
        ...
        if (store.mapped()) { // key based
            ...
            if (!digIndexBased(dig, groupID, store, rtvals))
                return false;
        }
        if (rtvals != null)
    ...
    private static boolean digIndexBased(XMLdigger dig, String groupID, ValStore store, Rtvals rtvals ) {
        ArrayList<BaseVal> localRtvals = new ArrayList<>();
        for (var val : dig.digOut("*")) {
            ...
            while (i >= localRtvals.size()) // Make sure the arraylist has at least the same amount
                localRtvals.add(null);

            final int pos = i; // need a final to use in lambda's
            if (localRtvals.get(i) != null) {
