...
public class ValFab {
    ...
    public static Optional<FlagVal> buildFlagVal(XMLdigger dig, String altGroup, Rtvals rtvals ) {
        ...
        alterFlagVal(dig,fv, rtvals );

        return Optional.of(fv);
    }
    public static void alterFlagVal(XMLdigger dig, FlagVal fv, Rtvals rtvals ) {
        var def = dig.attr("default", fv.defValue);

        fv.defValue( dig.attr("def", def) );
        fv.unit( dig.attr("unit", fv.unit()) );

        var raiseBlock = parseBlock( dig, "set",rtvals);
        var fallBlock = parseBlock( dig, "cleared",rtvals);
        var highBlock = parseBlock( dig, "stillhigh",rtvals);
        var lowBlock = parseBlock( dig, "stilllow",rtvals);

        fv.setBlocks(highBlock, lowBlock, raiseBlock, fallBlock);
    }
    private static AbstractBlock parseBlock( XMLdigger dig, String tag, Rtvals rtvals ){
        if( dig.hasPeek(tag) ){
            dig.usePeek();
            var block = TaskManagerFab.parseNodeList( dig,rtvals, rtvals.getEventLoop());
            dig.goUp();
            return block;
        }
        return null;
