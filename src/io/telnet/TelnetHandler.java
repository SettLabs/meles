...
public class TelnetHandler extends SimpleChannelInboundHandler<byte[]> implements Writable {
    ...
    public void channelActive(ChannelHandlerContext ctx) {
		...
		if( bootOk ) {
			...
			writeLine("Press <enter> to shut down meles...");
		}
	}
	private void readXMLsettings( ){
		var dig = XMLdigger.goIn(Paths.settings(), "meles", "settings", "telnet");
	...
	private void doStartCmd(String cmd){
		if (id.isEmpty() || id.equalsIgnoreCase("telnet")) {
			...
			writeLine(XMLfab.withRoot(Paths.settings(), "meles", "settings", "telnet").selectChildAsParent("client", "id", id)
					.map(f -> {
	...
	private void doMacroCmd( String macro ){
		if (!macro.contains("->")) {
			...
			var ma = macro.split("->");
			writeString(XMLfab.withRoot(Paths.settings(), "meles", "settings", "telnet").selectChildAsParent("client", "id", id)
					.map(f -> {
