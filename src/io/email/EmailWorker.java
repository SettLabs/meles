...
public class EmailWorker implements CollectorFuture, EmailSending, Commandable {
	...
	public boolean readFromXML(){
		...
		XMLdigger xml = XMLdigger.goIn(Paths.settings(), "meles", "settings", "email");
		if( !xml.isValid() )
			xml = XMLdigger.goIn(Paths.settings(), "meles", "email");
			...
			switch (xml.tagName("")) {
				case "outbox" -> {
					if (xml.hasPeek("server")) {
						xml.usePeek(); // Peek cancels itself after looking at it once, so do use
					...
					outbox.from = xml.peekAt("from").value("admin");// From emailaddress, default to admin
		...
		if( !outbox.from.contains("@") ) { // Meaning a ref instead of an address
			var temp = refToEmail(outbox.from);
			if( temp.isEmpty() ) {
				Logger.error("Used an unknown ref as 'from' for the outbox!");
			}else{
				outbox.from=temp;
			}
		}
	...
	public String replyToCommand(Datagram d) {
		...
		return switch (cmds[0]) {
			...
			case "send" -> {
				...
				sendEmail(Email.to(cmds[1]).subject(cmds[2]).content(cmds[3]).writable(d.getWritable()));
			...
			case "read" -> {
				addTarget(d.getWritable());
				yield "Added data request.";
			}
			case "read?" -> {
				checker.cancel(false);
				checker = scheduler.schedule(new Check(), 3, TimeUnit.SECONDS);
				d.getWritable().giveObject("alive",true);
				yield "ok";
			}
			case "","stop" -> removeWritable(d.getWritable())?"Ok.":"";
	...
	private String refToEmail( String ref ){
		String found = emailBook.get(ref);
		if( found ==null){
			Logger.error("Invalid ref given for email "+ref);
			return "";
		}
		return found;
	}
	...
	public synchronized boolean addTarget(Writable writable ){
		if( writable == null){
			Logger.error("Tried adding request to email but writable is null");
			return false;
		}
		if( targets.contains(writable)){
			Logger.info("email -> Already has "+writable.id()+" as target, not adding.");
			return true;
		}
		...
		Logger.info("Added request from "+writable.id()+ " to email");
		return true;
	}
	/* *********************************  W O R K E R S ******************************************************* */
	...
	private void sendEmail(Email email, boolean retry) {
		...
		try {
			...
			Logger.info("Email send to "+email.toRaw);
			email.inform(true);
			if( hasAttachment ){
				try {
	...
	public class Check implements Runnable {
		...
		public void run() {
			...
			if( !ok ){ // If no connection could be made schedule a sooner retry
				...
				maxQuickChecks--;
				Logger.info("Still got "+maxQuickChecks+" quicker checks to go, next in "+TimeTools.convertPeriodToString(Math.min(checkIntervalSeconds/3, 30),TimeUnit.SECONDS));
	...
	/**
	 * Check the origin of the email to see if that origin is actually allowed to talk to meles
	 * @param from The origin
	...
	private void processCommand( String cmd, String from, String body){
		...
		if( cmd.startsWith("label:")&&cmd.length()>7){ // email acts as data received from sensor, no clue on the use case yet
			...
			if( !targets.isEmpty() )
				targets.forEach( wr -> wr.writeLine("email",body ) );
		}
	}

	/**
	 * If the email has a 'for' in the subject it means it's for a certain instance of meles, check if this matches
	 * @param message The message received
	 * @param to The
	...
	private static String getTextFromMessage(Message message) throws MessagingException, IOException {
		...
		if (message.isMimeType("text/plain")) {
			...
			if( message.getContent() instanceof String res)
				return res.trim();
			return "";
	...
	private static class MailBox{
		...
		String from = "meles"; // The email address to use as from address
